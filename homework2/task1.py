...
def get_longest_diverse_words(file_path: str) -> List[str]:
    ...
    of unique symbols """
    # we will collect 'word : number of occasions' pairs in dict
    res_dict = {}
    res_list = []
    # trying to open file
    with open(os.getcwd() + file_path, "r") as fi:
        ...
        for line in fi:
            ...
            for word in line.split():
                ...
                if word not in res_dict.keys():
                    res_dict[word] = len(set(word))
        # getting 10 longest words from
        for counter in range(10):
            for key in res_dict.keys():
                if res_dict[key] == sorted(list(res_dict.values()),
...
def get_most_common_non_ascii_char(file_path: str) -> str:
    ...
    with open(os.getcwd() + file_path) as fi:
        ...
        for line in fi:
            ...
            for char in bytes(line, "ascii").decode("unicode-escape"):
                # got non-ascii
                if not char.isascii():
                    # new one
                    if char not in res_dict.keys():
